...
export function loadProfile3(res: LoadProfileResponse3) {
  ...
  encodeChara(res.chara).copy(buf, 0x0ac8);
  ...
  iconv.encode(res.name + "\0", "shift_jis").copy(buf, 0x0712);
  ...
  // Weekly Missions
  buf.writeUInt32LE((new Date(res.weeklyMissions.weeklyReset).getTime() / 1000) | 0, 0x1068);
  ...
  for (let i = 0; i < 6 && i < res.story.rows.length; i++) {
    const row = res.story.rows[i];
    const rowOffset = 0x0258 + i * 0x7a;

    for (let j = 0; j < 28 && j < row.cells.length; j++) {
